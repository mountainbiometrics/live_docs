...
### Pass 5: `index-moc` (index / Map-of-Content curation)

**Goal**: keep index docs healthy as navigational signposts — neither overloaded,
nor stale, nor pointing into void.  This pass is **proposal-only**: it never
rewrites doc bodies or edges without user confirmation.

The three sub-checks run together in one scan:

#### 5a. Oversized index (split signal)

An index that has grown too many children has become a drawer, not a map.
Threshold: > 10 direct children is a yellow flag; > 20 is a red flag.

```bash
python3 scripts/ldoc.py reindex          # regenerate the rollup first
cat kb/02-docs/.index/hierarchy.md             # read children counts per index
```

For each index above threshold, look at the child summaries in the index body
(between `<!-- BEGIN GENERATED CHILDREN -->` and `<!-- END GENERATED CHILDREN -->`
or the `## Children` section) and identify coherent sub-themes that could form a
child index.  Ask:

- Do groups of children share a domain, scope, lifecycle, or principle cluster?
- Would a reader naturally expect to find them under a named sub-index?

Propose: split the oversized index by adding one or more new `type: index` docs
and moving a logical subset of children to belong to the new sub-index via
`belongs_to` edges (not `requires`).  Show the proposed groupings as a table
before applying anything:

```
Index "<title>" has N children — proposed sub-indexes:
  Sub-index A: "<theme>"
    - <child-id> "<title>"
    - <child-id> "<title>"
  Sub-index B: "<theme>"
...
Do NOT auto-apply. Propose only.

#### 5b. Stale rollup (reindex signal)

A stale rollup occurs when the `<!-- BEGIN GENERATED CHILDREN -->` block in an
index body does not reflect the current hard-edge graph — for example, a doc has
added `belongs_to: [<idx-id>]` but the rollup has not been regenerated.

Detection:
```bash
python3 scripts/ldoc.py reindex --dry-run   # not yet implemented — use reindex + git diff instead
python3 scripts/ldoc.py reindex
git diff kb/02-docs/                         # if any index .md changed, rollup was stale
git checkout kb/02-docs/                     # restore (do not commit reindex output here)
```

If the diff shows any index doc body changed, report it as stale:

```
Stale rollup: <id>  "<title>"  — regenerate with `python3 scripts/ldoc.py reindex`
```

Propose: run `python3 scripts/ldoc.py reindex` (one command, no further edits).
This is a safe automated fix; propose it and apply immediately on confirmation.

**Note**: reindex is always idempotent — a second run after the first produces no
diff.  Use that as the staleness test: if the second run shows "unchanged" for all
index docs, the rollup is fresh.

#### 5c. Orphaned docs that belong to an index

Some non-index docs have no hard edges but their content clearly belongs under
an existing index.  Cross-reference with `kb/02-docs/.index/orphans.txt`:

```bash
cat kb/02-docs/.index/orphans.txt
```

For each orphan, read its summary and type:
```bash
python3 scripts/ldoc.py show <id>
```

If the orphan is a `type: type` or `type: index` doc it is graph-exempt and
should be skipped.  For genuine orphans, look at which existing index doc is the
natural parent (typically the root index, or a thematic sub-index if one exists).

Propose: add a `belongs_to` edge from the orphan to the index:
```bash
python3 scripts/ldoc.py link <orphan-id> --belongs-to <index-id>
python3 scripts/ldoc.py history <orphan-id> --add "linked to <index-id> — orphan resolved by garden index-moc pass"
```
